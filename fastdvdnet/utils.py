...
def normalize_augment(datain_original, datain_noisy, ctrl_fr_idx):
	...
	def transform(sample):
		...
		aug_list = [do_nothing]
		w_aug = [1] # one fourth chances to do_nothing
		transf = choices(aug_list, w_aug)

		# transform all images in array
		return transf[0](sample)

	img_train_original = datain_original
	# convert to [N, num_frames*C. H, W] in  [0., 1.] from [N, num_frames, C. H, W] in [0., 255.]
	img_train_original = img_train_original.view(img_train_original.size()[0], -1, img_train_original.size()[-2], img_train_original.size()[-1]) / 255.

	#augment
	img_train_original = transform(img_train_original)

	# extract ground truth (central frame)
	gt_train_original = img_train_original[:, 3*ctrl_fr_idx:3*ctrl_fr_idx+3, :, :]

	img_train_noisy = datain_noisy
	# convert to [N, num_frames*C. H, W] in  [0., 1.] from [N, num_frames, C. H, W] in [0., 255.]
	img_train_noisy = img_train_noisy.view(img_train_noisy.size()[0], -1, img_train_noisy.size()[-2], img_train_noisy.size()[-1]) / 255.

	# augment
	img_train_noisy = transform(img_train_noisy)

	# extract ground truth (central frame)
	gt_train_noisy = img_train_noisy[:, 3 * ctrl_fr_idx:3 * ctrl_fr_idx + 3, :, :]

	return img_train_original, img_train_noisy, gt_train_original, gt_train_noisy
